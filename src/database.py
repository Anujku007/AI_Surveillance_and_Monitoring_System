    ...
    with error_context("Initializing database"):
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS entry_exit_logs (
                ...
                snapshot_path   TEXT,
                is_suspicious   INTEGER DEFAULT 0,  -- 1 if unmatched/unknown
...
def log_event(person_id, event_type, camera_location="Main Entrance",
              snapshot_path=None, is_suspicious=False):
    """
    ...
    with error_context(f"Logging {event_type} event"):
        ...
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO entry_exit_logs (person_id, event_type, timestamp, camera_location, snapshot_path, is_suspicious)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (
            person_id,
            event_type,
    ...
    with error_context("Fetching recent logs"):
        ...
        cursor.execute("""
            SELECT l.log_id, l.event_type, l.timestamp, l.camera_location,
                   l.is_suspicious, l.snapshot_path,
                   p.name, p.identifier
